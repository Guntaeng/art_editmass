...
*** Keywords ***
No. 1 Edit field name :
    [Arguments]    ${field}
    IF         '${field}' == '${name_en.field}'
        Input text editmass row 1                           ${name_en.field}               ${name_en.value}
    ELSE IF    '${field}' == '${e_comm_name_th.field}'   
        Input text editmass row 1                           ${e_comm_name_th.field}        ${e_comm_name_th.value}
    ELSE IF    '${field}' == '${product_model.field}'   
        Input text editmass row 1                           ${product_model.field}         ${product_model.value}
    ELSE IF    '${field}' == '${tis_number.field}'   
        Input text editmass row 1                           ${tis_number.field}            ${tis_number.value}
    ELSE IF    '${field}' == '${tis_license_number.field}'   
        Input text editmass row 1                           ${tis_license_number.field}    ${tis_license_number.value}
    ELSE IF    '${field}' == '${selling_point.field}'   
        Input text editmass row 1 (section14)               ${selling_point.field}         ${selling_point.value}
    ELSE IF    '${field}' == '${maintenance.field}'   
        Input text editmass row 1 (section14)               ${maintenance.field}           ${maintenance.value}
    ELSE IF    '${field}' == '${method_of_use.field}'   
        Input text editmass row 1 (section14)               ${method_of_use.field}         ${method_of_use.value}
    ELSE IF    '${field}' == '${caution.field}'   
        Input text editmass row 1 (section14)               ${caution.field}               ${caution.value}
    ELSE IF    '${field}' == '${warranty_days.field}'   
        Input text editmass row 1 (day warranty)            ${warranty_days.field}         ${warranty_days.value}
    ELSE IF    '${field}' == '${item_code.field}'   
        Dropdown row 1                                      ${item_code.field}             ${item_code.value}
    ...
    ELSE IF    '${field}' == '${color_id.field}'   
        Dropdown row 1                                      ${color_id.field}              ${color_id.value}
    END

No. 2 Edit field name :
    [Arguments]    ${field}
    IF         '${field}' == '${name_en.field}'
...
Article Edit mass round 1
    ...
    No. 1 Edit field name :    ประเภทของการรับประกัน
    No. 2 Edit field name :    โทนสี
    No. 3 Edit field name :    จุดเด่นจุดขาย
    No. 4 Edit field name :    จำนวน(วัน)
    No. 5 Edit field name :    กำหนดรูปแบบการขาย


test keys
    Edit field 1    ชื่อสินค้า Commercial (ภาษาไทย)
